...
final class TestCaseMethodFactory
{
    ...
    public function buildForEvaluation(array $annotationsToUse, array $attributesToUse): string
    {
        ...
        $define_env_method = '';
        if($this->environment != []) {
            $define_env_method = $this->buildDefineEnvForEvaluation($this->environment[0]->name);
        }
     ...
     */
    private function buildDefineEnvForEvaluation(string $methodName): string
    {
        return <<<EOF
        
                public function define_env_$methodName(\$app)
                {
                    $methodName(\$app);
                }
